...
const AdminUsers = () => {
  ...
  const handleDelete = async (user) => {
    ...
    try {
      console.log('[DELETE USER] id:', user.id, 'role:', user.role);
      ...
      if (status === 404 || status === 405) {
        // Backend delete route may not be deployed yet — remove from UI
        toast.success(`${user.name || 'User'} removed. Redeploy backend for permanent deletion.`);
        setUsers((prev) => prev.filter((u) => !isSameUser(u, user)));
      } else {
...
const AdminDashboard = () => {
  ...
  const loadStats = async () => {
    ...
    try {
      ...
      if (Array.isArray(uData)) {
        allUsers = uData;
        allFarmers = uData.filter(u => u.role === 'farmer');
        allBuyers = uData.filter(u => u.role === 'buyer');
      } else {
        allFarmers = (uData.farmers || []).map(f => ({ ...f, name: f.name || `${f.first_name || ''} ${f.last_name || ''}`.trim(), role: 'farmer' }));
        allBuyers = (uData.buyers || []).map(b => ({ ...b, name: b.name || `${b.first_name || ''} ${b.last_name || ''}`.trim(), role: 'buyer' }));
        allUsers = [...allFarmers, ...allBuyers, ...(uData.users || [])];
      }
    } catch { /* silent */ }
  ...
  // Demo chart data (used when no API data)
  const dailySalesData = [
    { label: 'Mon', value: 4200 }, { label: 'Tue', value: 5800 },
    { label: 'Wed', value: 3900 }, { label: 'Thu', value: 7200 },
    { label: 'Fri', value: 6100 }, { label: 'Sat', value: 8500 },
  ...
  const monthlyRevenueData = [
    ...
    { label: 'May', value: 55000 }, { label: 'Jun', value: 72000 },
  ];

  const userRegistrations = [
    { label: 'Jan', value: 12 }, { label: 'Feb', value: 18 },
    { label: 'Mar', value: 24 }, { label: 'Apr', value: 31 },
    { label: 'May', value: 28 }, { label: 'Jun', value: 42 },
  ];

  const subscriptionGrowth = [
    { label: 'Jan', value: 3 }, { label: 'Feb', value: 5 },
    { label: 'Mar', value: 8 }, { label: 'Apr', value: 12 },
    { label: 'May', value: 15 }, { label: 'Jun', value: 22 },
  ];

  const categoryPerformance = [
    { label: 'Vegetables', value: 35 }, { label: 'Fruits', value: 28 },
    { label: 'Grains', value: 20 }, { label: 'Dairy', value: 12 },
    { label: 'Spices', value: 5 },
  ];
